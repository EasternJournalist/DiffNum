...
namespace Common {
	...
	struct mat : public array<array<T, NCol>, NRow>{
		...
		static const size_t _Size = NRow * NCol;
		...
		__HOST_DEVICE__ void fill(const T val) {
			for (size_t i = 0; i < _Size; i++)
				((T*)this)[i] = _Val;
		}
		...
		__HOST_DEVICE__ mat<T, NCol, NRow> inverse() const {
			...
			for (i = 0; i < NCol; i++) {
				for (firstrownot0 = i; firstrownot0 < NCol && T(0) == ma[firstrownot0][i]; firstrownot0++);
		...
		__HOST_DEVICE__ void SR(size_t i, size_t j) {
			for (size_t k = 0; k < NCol; k++) {
				T _Tmp = (*this)[i][k];
				(*this)[i][k] = (*this)[j][k];
				(*this)[j][k] = _Tmp;
		...
		//把行 j 的 val 倍加到行 i 上
		__HOST_DEVICE__ void TR(size_t i, size_t j, const T val) {
			for (size_t k = 0; k < NCol; k++) {
				(*this)[i][k] += val * (*this)[j][k];
			}
		}
		//把行 i 乘以 val 倍
		__HOST_DEVICE__ void DR(size_t i, const T val) {
			for (size_t k = 0; k < NCol; k++) {
				(*this)[i][k] *= val;
			}
		}
		...
		__HOST_DEVICE__ constexpr size(void) {
			return _Size;
		}

		__HOST_DEVICE__ T* data(void) const {
			return (T*)this;
		}
